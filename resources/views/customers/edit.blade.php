        ...
        <div class="card">
            ...
            <div class="card-body p-4">
                <form action="{{ route('clientes.update', $customer->id) }}" method="POST"
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-6">
                            <div class="mb-3">
                                ...
                                <input class="form-control {{ $errors->has('rut') ? 'is-invalid' : '' }}" type="text" name="rut" id="rut" required
                                    value="{{ $customer->rut }}" placeholder="00000000-0">
                                @if($errors->has('rut'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('rut') }}
                                    </div>
                        ...
                        <div class="col-lg-4 col-md-4 col-sm-6">
                            <div class="mb-3">
                                ...
                                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" required
                                    value="{{ $customer->email }}" placeholder="test@example.com">
                                @if($errors->has('email'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('email') }}
                                    </div>
                    ...
                    <button type="submit" class="btn btn-primary w-md float-end">Actualizar Clientes</button>
                </form>
            </div>
        </div>
    </div>
